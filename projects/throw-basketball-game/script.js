document.addEventListener("DOMContentLoaded", function (event) {
  window.onload = function () {
    ...
    gsap.to(".stage", { autoAlpha: 1, delay: 1 });
    ...
    function resize() {
      // For some reason, we need to re-add the touch events every time the orientation changes,
      // if we don't, the touchmove fails after the touchstart.
      removeEvents();
      addEvents();
      ...
      basketWidth = Math.round(
        basket.querySelector("rect").getBoundingClientRect().width
      );

      // Work out the ratio between the basket's width and the ball's radius, make it slightly smaller for safety
      ratio = basketWidth / ballRadius - 0.1;

      w = window.innerWidth;
      h = window.innerHeight;

      // Make sure the basketball has no previous GSAP transforms on it
      gsap.set(ball, { clearProps: "all" });

      // Move the basketball to its starting offset
      gsap.set(ball, { y: "+=" + offsetY });

      scale = gsap
        .to(ball, { duration: 0.5, scale: ratio, ease: "power1.inOut" })
        .progress(1)
        .pause(0);
    }
    ...
    function moveBall(e) {
      ...
      gsap.set(ball, { x: p.position.getX(), y: p.position.getY() });
    }
    ...
    function releaseBall() {
      ...
      // Limit how hard the ball can be thrown. Improves user accuracy and diminishes realistic movement.
      ...
      // Start GSAP's ticker so more physics-like movement can take place
      gsap.ticker.add(tick);

      // Stop it after some period of time - saves having to write edges and floor logic,
      // and the user can shoot every three seconds or so
